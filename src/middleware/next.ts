...
import { generateHalter } from './halt'
/**
 ...
 */
export function generateNextExecutor(list: TMiddleware[], offset: number, context: any, recorder: boolean[]) {
    async function executeNext() {
        if (list[offset] instanceof Function && !recorder[offset]) {
            recorder[offset] = true
            await list[offset](context, generateNextExecutor(list, offset + 1, context, recorder), generateHalter())
        }
    }
    return executeNext
}