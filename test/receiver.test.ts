/// <reference types="jest" />
import { Receiver } from '../src/classes/receiver'
import axios from 'axios'
import { createHmac } from 'crypto'
const PORT_OFFSET = 1
let receiver: Receiver
const secret = Math.random().toString()

test('Create Receiver', () => {
    expect(() => receiver = new Receiver(secret)).not.toThrow()
})

test(`Listen on ${8080 + PORT_OFFSET}`, () => {
    expect(() => receiver.listen(8080 + PORT_OFFSET)).not.toThrow()
})

test('Return 401 When No X-Signature', async () => {
    expect.assertions(1)
    try { await axios.post(`http://localhost:${8080 + PORT_OFFSET}`) }
    catch (err) { expect(err.response.status).toBe(401) }
})

test('Return 403 When Wrong X-Signature', async () => {
    expect.assertions(1)
    try { await axios.post(`http://localhost:${8080 + PORT_OFFSET}`, {}, { headers: { 'x-signature': 'sha1=whatever' } }) }
    catch (err) { expect(err.response.status).toBe(403) }
})

test('Handle Event Types', async () => {
    expect.assertions(4)
    const post = {
        post_type: 'fake_post',
        meta_event_type: 'unit_test',
        sub_type: 'fake_sub_type',
    }
    receiver.once('post', ctx => expect(ctx).toEqual(post))
    receiver.once('fake_post', ctx => expect(ctx).toEqual(post))
    receiver.once('fake_post/unit_test', ctx => expect(ctx).toEqual(post))
    receiver.once('fake_post/unit_test/fake_sub_type', ctx => expect(ctx).toEqual(post))
    await axios.post(`http://localhost:${8080 + PORT_OFFSET}`, post, { headers: {'x-signature': `sha1=${createHmac('sha1', secret).update(JSON.stringify(post)).digest('hex')}`} })
})